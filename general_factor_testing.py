...
#     # dummy_var = get_industry_exposure(industry_label.columns, industry_label, date).T
#     x = dummy_var.loc[index]
#     x = pd.concat([np.log(market_value,x)],axis=1)   # 市值中性化+行业中性化
#     result = sm.OLS(y.astype(float), x.astype(float)).fit()
#     residue = result.resid
#     return residue
...
if __name__ == '__main__':
    # MV因子
    print("-------------MV因子测试开始----------")
    f_date_number = datenum_to_list(factor)
    new_date = date
    factor = pd.DataFrame(factor['MV'],columns=array_to_list(factor,'RICs'))  # 读取MV因子值
    factor = factor[0:len(new_date)]
    factor.index = new_date['date']
    stocks=factor.columns & stock(DayPrice_stock).columns    # 取原股票池和市值因子MV股票池的交集
    factor=factor[stocks]
    # print("取股票池交集后，市值因子MV的因子DateFrame如下：")
    # print((factor_MV))                                        # 初始因子值DataFrame
    # print("总期数："+str(len(factor_MV)))
    # print("股票池数量："+str(len(factor_MV.columns)))
    ...
    # time_start = time.time()
    print("行业中性化开始")
    new_factor_df = set_new_factor_df(standardize_factor, industry_label)
    # new_factor_df.to_csv('factor_neu.csv')
    print("行业中性化结束")
    # time_end = time.time()
    ...
    # date_return_df.to_csv("date_return_df.csv")
    ...
    # net_value_df.to_csv("net_value.csv")
    ...
    time_end = time.time()
    print('程序运行共需' + str(time_end - time_start) + '秒')
